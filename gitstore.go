...
func (repo *LocalGitRepo) ListDrawings(ctx context.Context) (map[string]string, error) {
	...
	for _, key := range keys {
		...
		fmt.Printf("key[0:len(repo.pathToDrawings)]: %v, repo.pathToDrawings: %v\n", key[0:len(repo.pathToDrawings)], repo.pathToDrawings)

		if key[0:len(repo.pathToDrawings)] != repo.pathToDrawings {
			continue
		}

		fmt.Printf(">>>>>>>>>> got: key[0:len(repo.pathToDrawings)]: %v, repo.pathToDrawings: %v\n", key[0:len(repo.pathToDrawings)-1], repo.pathToDrawings)

		peeledBackKey := key[len(repo.pathToDrawings):]
		...
		drawingList[key[len(repo.pathToDrawings)+1:]] = title
	}
